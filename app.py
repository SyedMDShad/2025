...
def compute_model(out_p, in_p, main_p, out_v, in_v, main_v, live):
    ...
    if solar_mode == "manual":
        ...
        # solar first (what the array covers), then battery flow = leftover balance
        pv0 = min(max(out_p + ovh - in_p, 0.0), ARRAY_WATT) if sun_el > 2 else 0.0
        balance = in_p + pv0 - out_p - ovh             # >0 grid surplus, <0 load shortfall
        if full:
            ch_w, dis_w = 0.0, 0.0                     # full battery: everything = solar + own use
        elif balance > 15:
            ch_w, dis_w = min(balance, grid_cap_w), 0.0   # surplus -> battery (cap 10A)
        elif balance < -15:
            ch_w, dis_w = 0.0, -balance                # battery covers the shortfall
        else:
            ch_w = dis_w = 0.0                         # within inverter overhead band
        # final solar reading from the meters: in + pv = out + ovh + ch - dis
        pv_m = out_p + ovh + ch_w - dis_w - in_p
        solar_pv_w = round(min(max(pv_m, 0.0), ARRAY_WATT), 1) if (sun_el > 2 and pv_m > 5) else 0.0
    else:
    ...
    return {"pv": solar_pv_w, "ch": ch_w, "dis": dis_w, "amps": current_amp,
            "full": full, "ovh": ovh, "is_grid": is_grid, "sun_el": sun_el}
...
def ha_states():
    ...
    states = [
        ...
        {"entity_id": "sensor.energy_mate_v4_pack_version", "state": "V12.1", "last_updated": now_iso},
