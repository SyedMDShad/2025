...
diag = {"last_error": "never polled yet", "last_ok": 0.0, "quota_exhausted": False}
...
def fetch_single_device(device_id):
    ...
    try:
        ...
        msg = str(res.get("msg", ""))
        if res.get("code") == 28841004 or "quota" in msg.lower():
            diag["last_error"] = "Tuya IoT Core quota exhausted: " + msg
            diag["quota_exhausted"] = True
        else:
            diag["last_error"] = "device API: " + msg
            diag["quota_exhausted"] = False
    except urllib.error.HTTPError as e:
        if e.code == 401:
...
def poll_round():
    ...
    if got:
        ...
        if not diag.get("quota_exhausted"):
...
def background_tuya_poller():
    ...
    while True:
        ...
        if diag.get("quota_exhausted"):
            time.sleep(300)      # don't hammer a quota-blocked account
        else:
            time.sleep(15 if fails < 3 else 30)
...
def ha_states():
    ...
    states = [
        ...
        {"entity_id": "sensor.energy_mate_v4_pack_version", "state": "V13.5", "last_updated": now_iso},
...
def healthz():
    ...
    return jsonify({"ok": True, "tuya_connected": conn, "last_device_update_age_s": age,
                    ...
                    "quota_exhausted": bool(diag.get("quota_exhausted", False)),
