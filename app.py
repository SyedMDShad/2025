...
INV_OVH = 45.0              # inverter self-consumption (W) — always between in and out
INV_GRID_CHARGE_MAX = 250.0 # inverter grid->battery charge ceiling (W) when user cap = 0
MPPT_MAX = 280.0            # solar MPPT -> battery ceiling (W)
...
user_settings = {
    ...
    "battery_amps": 0.0,          # 0 = auto (inverter charge ceiling); >0 = your measured charge cap (A)
...
def ha_states():
    ...
    grid_charge_max = (max(user_settings["battery_amps"], 0.0) * 13.5) if user_settings["battery_amps"] > 0 else INV_GRID_CHARGE_MAX
    ...
    # When the battery is FULL the inverter does NOT charge it — the leftover
    # (in - out - ovh) is inverter self-consumption, so ch/dis = 0, amps = 0.
    if solar_mode == "manual":
        # explicit override mode (user's choice)
        solar_pv_w = round(manual_w, 1)
        ch_w = round(grid_charge_max, 1) if not full else 0.0
        dis_w = 0.0
    elif not live:
        solar_pv_w = ch_w = dis_w = 0.0
    elif is_grid:
        surplus = in_p - out_p - INV_OVH
        if full:
            ch_w = dis_w = 0.0                    # full: surplus = inverter own use
        elif surplus > 15:
            ch_w, dis_w = min(surplus, grid_charge_max), 0.0   # grid surplus -> battery
        elif surplus < -15:
            ch_w, dis_w = 0.0, -surplus           # grid can't cover load -> battery
        else:
            ch_w = dis_w = 0.0                    # within inverter overhead band
        solar_pv_w = round(min(pv_dc, MPPT_MAX), 1) if (sun_el > 2 and not full) else 0.0
    else:
        # grid off: daytime = solar feeds the load + charges battery; night = battery feeds the load
        if sun_el > 2:
            dis_w = 0.0
            ch_w = 0.0 if full else min(MPPT_MAX, pv_dc)
            solar_pv_w = round(out_p + INV_OVH + ch_w, 1)
        else:
            dis_w = out_p + INV_OVH
            ch_w = 0.0
            solar_pv_w = 0.0
    ...
    states = [
        ...
        {"entity_id": "sensor.energy_mate_v4_inverter_overhead_power", "state": str(int(INV_OVH)), "last_updated": now_iso},
        {"entity_id": "sensor.energy_mate_v4_pack_version", "state": "V10.4", "last_updated": now_iso},
...
def api_settings():
    if request.method == "POST":
        ...
        for field in ("battery_soc", "battery_amps", "battery_capacity_ah", "manual_solar_watts", "tariff_rate"):
            if field in data:
                try:
