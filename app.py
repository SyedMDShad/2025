...
INV_OVH = 45.0              # inverter self-consumption (W)
...
user_settings = {
    ...
    "battery_amps": 21.0,         # MPPT charge cap (A) used for solar estimate + cap
...
def ha_states():
    ...
    ch_cap_w = max(user_settings["battery_amps"], 0.0) * 13.5     # MPPT charge cap (W)
    ...
    if solar_mode == "manual":
        ...
        if surplus > 10:
            ch_w, dis_w = surplus, 0.0          # grid surplus -> battery
        elif surplus < -10:
            ch_w, dis_w = 0.0, -surplus         # grid can't cover load -> battery
        else:
            ch_w = dis_w = 0.0
        # solar (DC side, not on AC meters): MPPT estimate, capped, stops at full
        if sun_el > 2 and soc_state["soc"] < 99.5:
            pv_dc = ARRAY_WATT * (dayF ** 1.3) * 0.85
            solar_pv_w = round(min(pv_dc, ch_cap_w), 1)
        else:
            solar_pv_w = 0.0
    else:
        # grid off: battery powers the load; solar (MPPT) charges the battery
        dis_w = out_p + INV_OVH
        if sun_el > 2 and soc_state["soc"] < 99.5:
            pv_dc = ARRAY_WATT * (dayF ** 1.3) * 0.85
            ch_w = min(ch_cap_w, pv_dc)
            solar_pv_w = round(out_p + INV_OVH + ch_w, 1)
        else:
            ch_w = 0.0
            solar_pv_w = 0.0
            if sun_el > 2 and out_p > 5:
                # sun is up but battery full / no charge — show what's feeding the load
                solar_pv_w = round(out_p + INV_OVH, 1)

    batt_v = user_settings.get("battery_voltage", 13.5)
    if ch_w >= dis_w:
        current_amp = round(ch_w / batt_v, 1) if ch_w > 3 else 0.0
    else:
    ...
    states = [
        ...
        {"entity_id": "sensor.energy_mate_v4_pack_version", "state": "V10.3", "last_updated": now_iso},
